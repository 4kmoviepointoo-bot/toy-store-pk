...
function safeImageUrl(src: string): string {
  if (!src || isBase64(src)) return `${SITE_URL}${PLACEHOLDER_IMG}`;
  return `${SITE_URL}${src}`;
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  try {
    ...
    if (dbProduct) {
      ...
      const imageUrl = safeImageUrl(dbProduct.image);
    ...
    console.error("[ProductPage] generateMetadata failed:", err);
    return {
      title: "Product — ToyVerse Pakistan",
      description: "Browse premium toys for kids in Pakistan. Cash on delivery available.",
    };
  }
}
